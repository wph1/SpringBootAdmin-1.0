...
* @js调用方法：
* verifyCheck({
*  	formId:'verifyCheckSubnet',		<验证formId内class为required的元素
...
(function($) {
	...
	function require(f) {
		var g = {
			...
			uname: /^[a-zA-Z]\w*$/,
			address:/^((25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))\.){3}(25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))$/,
			zh: /^[一-龥]+$/,
		...
		this.rules = {
			...
			isAddress: function(a, b) {
				b = b || " ";
				if (!g.address.test(a)) return b
			},
			isZh: function(a, b) {
				b = b || " ";
	...
	require.prototype = {
		_init: function(b) {
			this.config = b;
			this.getInputs = $('#' + b.formId).find('.requiredMY:visible');
			var c = false;
			var d = this;
			if (b.code) {
	...
	j._click = function(c) {
		c = c || opt.formId;
		var d = $("#" + c).find('.requiredMY:visible'),
			self = this,
			result = true,
			t = new require(),
			r = [];
	...
	window.verifyCheckSubnet = $.verifyCheckSubnet = j
})(jQuery);
(function($) {
	var f;
	var g = function() {
...
$(function() {
	...
	$('body').on("keyup", "#password", function() {
		...
		if (t.length >= 6) {
			o.show();
			var l = verifyCheckSubnet.pwdStrong(t);
